...
export type SetTweak = (keyOrEdits: string | Record<string, unknown>, val?: unknown) => void

export type RadioOptionValue = string | number | boolean
export type RadioOption = RadioOptionValue | { label: string; value: RadioOptionValue }
...
export function useTweaks<T extends object>(defaults: T): [T, SetTweak] {
  const [values, setValues] = React.useState<T>(defaults)

  const setTweak = React.useCallback((keyOrEdits: string | Record<string, unknown>, val?: unknown) => {
...
interface TweakRadioProps {
  ...
  options: RadioOption[]
  onChange: (v: RadioOptionValue) => void
}

export function TweakRadio({ label, value, options, onChange }: TweakRadioProps) {
  const trackRef = React.useRef<HTMLDivElement>(null)
  const [dragging, setDragging] = React.useState(false)
  const valueRef = React.useRef<RadioOptionValue>(value)
  valueRef.current = value

  const labelLen = (o: RadioOption): number =>
  ...
  const fitsAsSegments =
    maxLen <= (({ 2: 16, 3: 10 } as Record<number, number | undefined>)[options.length] ?? 0)

  if (!fitsAsSegments) {
    const resolve = (s: string): RadioOptionValue => {
  ...
  const opts = options.map((o): { value: RadioOptionValue; label: string } =>
    typeof o === 'object' ? o : { value: o, label: String(o) }
  )
  const idx = Math.max(0, opts.findIndex((o) => o.value === value))
  const n = opts.length

  const segAt = (clientX: number): RadioOptionValue => {
